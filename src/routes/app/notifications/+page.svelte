    ...
    <div class="notifcol flex flex-col-reverse pl-4 pt-6 pr-4">
        {#each $notificationHistory as notif}
            <div class="notifs flex items-center">
                <div class={"iconbox rounded-full p-1 ml-4 mb-8 flex-shrink-0 " + (notif.type === "softlimit" ? "bg-[#F06346]" : notif.type === "scheduled" ? "bg-[#F0A346]" : "bg-[#F04646]")}>
                ...
                <div class="flex flex-col ml-2 p-1 pl-4 pr-12 w-full">
                    ...
                    <div class="message text-base leading-tight pt-1 pr-4">Socket {notif.socket} {notif.message}</div>
                    {:else if notif.type === "softlimit"}
                    <div class="message text-base leading-tight pt-1 pr-4">{notif.message} {$softlimitThreshold} kWh.</div>
                    {:else}
