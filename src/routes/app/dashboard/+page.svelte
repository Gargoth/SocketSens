...
<div class='power'>
  ...
  <div class="bg-gradient-to-r from-orange-500/[.8] to-orange-700/[.8] rounded-2xl h-24 mx-4 p-3 px-4">
    <span class="bg-white rounded-full px-2 py-1 text-orange-600 text-xs">Total Energy Consumption</span>
...
<div class="mt-6">
  ...
  <div class="thingy grid grid-cols-2 gap-4 p-4 pt-6">
    <Socket socketNum={1} socketName={'Electric Fan'} socketPwr={0.15} isSocketOn={$toggles[0]} on:toggle={()=>changeWaitingStates(0)} isSocketWaiting={$isWaiting[0]}/>
    <Socket socketNum={2} socketName={'Lamp'} socketPwr={0.35} isSocketOn={$toggles[1]} on:toggle={()=>changeWaitingStates(1)} isSocketWaiting={$isWaiting[1]}/>
    <Socket socketNum={3} socketName={'Charger'} socketPwr={0.1} isSocketOn={$toggles[2]} on:toggle={()=>changeWaitingStates(2)} isSocketWaiting={$isWaiting[2]}/>
    <Socket socketNum={4} socketName={'Hairdryer'} socketPwr={0.5} isSocketOn={$toggles[3]} on:toggle={()=>changeWaitingStates(3)} isSocketWaiting={$isWaiting[3]}/>
  </div>
</div>
